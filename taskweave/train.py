...
from taskweave.config import BaseOptions
from taskweave.start_end_dataset import \
    StartEndDataset, start_end_collate, prepare_batch_inputs
from taskweave.start_end_dataset_audio import \
    StartEndDataset_audio, start_end_collate_audio, prepare_batch_inputs_audio
from taskweave.inference import eval_epoch, start_inference, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    for batch_idx, batch in tqdm(enumerate(train_loader),
        ...
        timer_start = time.time()
        outputs,[hd_log_var,mr_log_var] = model(epoch_i,**model_inputs)
        loss_dict = criterion(outputs, targets)
        weight_dict = criterion.weight_dict

        tb_writer.add_scalar("hd_log_var", float(hd_log_var), epoch_i+1)
        tb_writer.add_scalar("mr_log_var", float(mr_log_var), epoch_i+1)
        
        grouped_losses = {"loss_mr": [], "loss_hd": []}
        for k in loss_dict.keys():
            if k in weight_dict:
                if any(keyword in k for keyword in ["giou", "span", "label",'class_error']):
                    # grouped_losses["loss_mr"].append(loss_dict[k] * weight_dict[k])
                    grouped_losses["loss_mr"].append(loss_dict[k])
                elif "saliency" in k:
                    # grouped_losses["loss_hd"].append(loss_dict[k] * weight_dict[k])
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    if opt.model_ema:
        print("\033[1;91mUsing model EMA ...\033[0m")
            ...
            with torch.no_grad():
                if opt.model_ema:
                    metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths = \
                        eval_epoch(model_ema.module, val_dataset, opt, save_submission_filename, epoch_i, criterion, tb_writer)
                else:
                    metrics_no_nms, metrics_nms, eval_loss_meters, latest_file_paths = \
